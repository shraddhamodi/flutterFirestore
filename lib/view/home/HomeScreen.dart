...
import '../../view_model/home/MoviesListVM.dart';
...
class _HomeScreenState extends State<HomeScreen> {
      ...
      body: ChangeNotifierProvider<MoviesListVM>(
        ...
        child: Consumer<MoviesListVM>(builder: (context, viewModel, _) {
          switch (viewModel.movieMain.status) {
            case Status.LOADING:
              print("HomeScreen Shraddha :: LOADING");
              return LoadingWidget();
            case Status.ERROR:
              print("HomeScreen Shraddha :: ERROR");
              return MyErrorWidget(viewModel.movieMain.message ?? "NA");
            case Status.SUCCESS:
              print("HomeScreen Shraddha :: COMPLETED");
  ...
  Widget _getMoviesListView(List<Employee>? moviesList) {
  ...
  Widget _getMovieListItem(Employee item) {
    return Card(
      child: ListTile(
        leading: ClipRRect(
          child: Image.network(
        ...
        title: MyTextView(item.name ?? "NA",context.resources.color.colorPrimaryText,context.resources.dimension.bigText),
        subtitle: MyTextView(item.salary.toString(),context.resources.color.colorSecondaryText,context.resources.dimension.mediumText),
        trailing: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            MyTextView("${Utils.setAverageRating(
                // item.ratings ??
                    [])}",context.resources.color.colorBlack,context.resources.dimension.mediumText),
  ...
  void _sendDataToMovieDetailScreen(BuildContext context, Employee item) {
